...
createApp({
  setup() {
    ...
    const mapCategory = (category) => {
      switch (category) {
        case 'Kick Out Plans': return 'kickOutPlans';
        case 'Income Plans': return 'incomePlans';
    ...
    const fetchProducts = async (direction = '') => {
      loading.value = true;
      let nextPageParams = direction && cursorPointer ? `&nextCursorPointer=${encodeURIComponent(cursorPointer)}&direction=next` : '';
